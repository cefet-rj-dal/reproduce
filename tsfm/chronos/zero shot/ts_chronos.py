...
class TsChronosNet:
    def __init__(
        ...
        Parameters
        - df_test: DataFrame with the full time series, including the
          prediction context and the forecast tail at the end.
        - data_target: Name of the target column in df_test.
        - model: Pre-trained Chronos model to use. One of
          ["amazon/chronos-t5-tiny", "amazon/chronos-t5-small",
           "amazon/chronos-t5-base", "amazon/chronos-t5-large"].
        - device_map: Device to run the model on ("cuda", "cpu", or "mps").
        - torch_dtype: Data type for torch tensors.
        - prediction_length: Number of future time steps to predict.
        - num_samples: Number of samples to draw for each prediction.
        - temperature: Sampling temperature.
        - top_k: Top-K sampling.
        - top_p: Nucleus sampling probability.
        - limit_prediction_length: Whether to limit the prediction length.
        """
        ...
        # Prepare the prediction context by removing the forecast tail
...
def ts_chronos_create(df=pd.DataFrame(), target=[], pred_length=0, samples=100, base="amazon/chronos-t5-small"):
    """Helper to build a TsChronosNet with sensible defaults."""
    model = TsChronosNet(
        df_test=df,
        data_target=target,
...
def ts_chronos_fit(model, df_train):
    # Chronos pipeline is used in zero-shot; nothing to fit here.
    return model


def ts_chronos_predict(model, df_test, target, pred_length=1, samples=100, base="amazon/chronos-t5-small") -> pd.DataFrame:
    """Generate predictions using the Chronos model (zero-shot)."""
    ...
    # Example: compute the median forecast (not returned here)
