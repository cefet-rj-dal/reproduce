##
## Harbinger v1: Event Detection Helpers
## -------------------------------------
## This module provides a set of wrappers and optimizers for event detection
## over time series, unifying call signatures and outputs.
##
## Conventions
## - Input `data`: data.frame with a first `time` column and one series column
##   (or more; when more than one, detectors may treat them as multivariate).
## - Detector `func`: returns a data.frame with columns:
##   `time` (event timestamp/index), `serie` (series name), `type` (event type),
##   where `type` is one of "anomaly" or "change point".
## - Output: a data.frame concatenating detected events following the schema
##   above. Downstream evaluators (e.g., SoftED) expect this structure.
##
#==== evtdet: generic dispatcher ====
# input:
...
##
## evtdet.anomalize: STL/Twitter seasonal decompose + IQR/GESD anomaly flagging
## -----------------------------------------------------------------------------
## Uses the anomalize package to decompose and flag anomalies on a single series.
## Parameters allow choosing the decomposition method and anomaly test.
...
##
## optim.evtdet.anomalize: grid search over anomalize parameters
## -------------------------------------------------------------
...
##
## evtdet.eventdetect: EventDetectR wrapper
## ---------------------------------------
...
##
## evtdet.seminalChangePoint: sliding-window regression split (v1)
## ----------------------------------------------------------------
...
##
## evtdet.seminalChangePoint2: AR(P) windowed split evaluation (v2)
## ----------------------------------------------------------------
...
##
## evtdet.changeFinder: change finder with ARIMA/ETS smoothing (v3)
## ---------------------------------------------------------------
