##
## Multi-Scale Event Detection (core pipeline)
## -------------------------------------------
## Purpose:
## - Decompose a time series via CEEMD and derive component groups:
##   short-term fluctuation (stf), shock/significant events (sse),
##   volatility events (ve), overall adaptive trend (oat), long-term (lt)
## - Detect events by type: anomalies, change points (cp), cp_variance (ICSS)
## - Optionally compute SoftED-style soft matching for detected event times
## - Return evaluation metrics against a reference dataframe (time,event)
##
## Dependencies: componentes.R (CEEMD + energy ratio), utils.R (helpers),
## functions-models.R (soft evaluation helpers), anomalies.R (anomaly wrapper)
...
soft_cp_breaks <- function(serie_tmp = serie_tmp, cp_detect = cp_detect, soft_window = soft_window) {
  ...
  # Soft matching around CP detections when available
  if (!is.null(cp_detect) && !is.null(cp_detect$SSCP) && nrow(cp_detect$SSCP) > 0) {
    SSCP_soft <- soft_evaluate_components(get_events(cp_detect$SSCP$time), serie_tmp, soft_window)
  }

  if (!is.null(cp_detect) && !is.null(cp_detect$STCP) && nrow(cp_detect$STCP) > 0) {
    STCP_soft <- soft_evaluate_components(get_events(cp_detect$STCP$time), serie_tmp, soft_window)
  }

  if (!is.null(cp_detect) && !is.null(cp_detect$LT) && nrow(cp_detect$LT) > 0) {
...
multi_scale_event_detect <- function(serie = NULL, noise.amp = 6.4e-07, trials = 100, limit_ri_sup = 2.14, limit_ri_inf = 1.50, frequency_date = 12, start_date = c(2004, 1), stop_rule = "type1", type_events = list("anomaly", "cp", "cp_variance"), rf_dataframe = NULL, soft_window = 15) {
  ...
  time <- detect_events
  # If no detections, build an empty events data.frame (handled below)
